...
#define RUN_SELFTEST(test_name, input_file, waiting_code) \
    ...
    RUN_SELFTEST("TEST2", "./test/global/test1.txt", TASK1_CODE_INCORRECT_INPUT_FILE);
...
//
    ...
    // Автоматизированное тестирование реализации функции
    //
    printf("CURRENT: TASK-2 SELFTEST\n");
...
#define RUN_SELFTEST(test_name, input_file, x_value, N_value, waiting_code) \
    ...
    // RUN_SELFTEST("TEST3", "./test/global/test1.txt", 1.9, 100500, TASK2_CODE_INCORRECT_INPUT_FILE);
...
#undef RUN_SELFTEST
    ...
    printf("CURRENT: TASK-2 UI\n");
    
    printf("File name:\n");
    if(scanf("%s", filein) != 1)
    {
	ERR_LOGGING(TASK2_CODE_NO_INPUT_FILE, "No input file %s", filein);
    ...
    if(scanf("%lf", &x) != 1)
    {
	ERR_LOGGING(TASK2_CODE_INCORRECT_ADV_DATA, "Incorrect input x");
    ...
    if(scanf("%d", &N) != 1)
    {
	ERR_LOGGING(TASK2_CODE_INCORRECT_ADV_DATA, "Incorrect input N");
	return TASK2_CODE_INCORRECT_ADV_DATA;
    }
